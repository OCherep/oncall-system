...
**Не публікується.** У `docker-compose.yml` лише `85:443`. У `nginx.conf` лише `listen 443 ssl` (без HTTP :80).
ACME/оновлення сертифікатів — через devops-hub або копіювання PEM у `./certs`.
На хості після `docker compose up` перевірте: `ss -lntp | grep -E ':80|:85'` — має бути лише 85.

## Порт 80

З **1.1.0** host port **80 не публікується** (зайнятий devops-hub). ACME/оновлення сертифікатів — через hub або ручне копіювання PEM у `./certs`.
