...
func handleGetData(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query(`SELECT u.id, u.username, u.name, u.role, u.team_role_id, COALESCE(tr.name, ''), COALESCE(u.is_oncall, 1)
		FROM users u LEFT JOIN team_roles tr ON u.team_role_id = tr.id WHERE COALESCE(u.is_oncall, 1) = 1 ORDER BY u.name`)
	var team []User
	...
	if len(shifts) == 0 {
		...
		for _, s := range shifts {
			db.Exec(`INSERT OR IGNORE INTO shifts (date, primary_user, backup_user) VALUES (?, ?, ?)`, s.Date, s.PrimaryUser, s.BackupUser)
		}
	}
...
func handleIncidents(w http.ResponseWriter, r *http.Request) {
	...
	// resolve role from DB if not provided
	role := inc.Role
	if role == "" {
	...
	// future date → also create daily task "для розгляду на дейлі"
	if isFuture {
...
func handleDailyTasks(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut:
		...
		if newStatus == "Перевідкрита" {
			...
			newStatus = "Нова" // equals Нова but keeps total_minutes
		}
		if cur.Status == "Виконана" && newStatus != "Нова" && newStatus != "Перевідкрита" && newStatus != "Архів" {
			// only reopen or archive from done
		...
		// Виконана → can stay; archive explicit
