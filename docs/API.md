...
## 10. Статуси (довідково)

### Звернення

`Нове` → `В роботі` → `На паузі` → `Вирішено` → `Архів`

### Задачі

`Нова` → `У роботі` → `На паузі` → `До перевірки` → `Виконана` → `Перевідкрита` / `Архів`  
Переходи обмежені роллю (admin ширше).

### Jira ↔ локальні (орієнтовно)

Env `JIRA_STATUS_MAP` JSON, напр.:
```json
{"Нове":"Open","В роботі":"In Progress","На паузі":"On Hold","Вирішено":"Done","Архів":"Done"}
```
Зміна статусу incident/task з `external_id` може викликати transition + comment у Jira.
...
- Нові задачі (конвертація зі звернення, Jira import, створення без статусу) стартують як **Нерозподілена**.
